...
该工具报告漂移，但不作为提交的门禁。内置漂移并非当前评审的改动所导致，而一次网络调用也不能待在 `hygiene` 或 pre-commit 钩子里。把它做成由维护者或计划任务调用的命令，能让离线门禁保持离线。`.github/workflows/vendor-drift.yml` 于每周一运行 `--check`：昨天动过的上游不必今天就回答，而每次推送都跑的任务会在有人处理之前一遍遍给出相同答案——信号正是这样被忽略的。
...
- 现在九行全部可解析且报告为最新，因此命令以零码退出，每周任务上线即为绿色。某一行若不再可解析，会成为比较失败，其失败 `--check` 的方式与落后相同。
- 每周任务变红是一条 TODO，而不是构建损坏：它意味着上游前进了，修复手段是同步流程而非回退。没有别的东西依赖该任务，因此它无法阻塞发布。
