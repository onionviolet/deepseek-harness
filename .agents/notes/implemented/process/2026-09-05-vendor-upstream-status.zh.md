...
其中两个记录的上游曾经根本无法访问。`cosmokit/` 与 `schemastery/` 行原先指向的 `deepseek-harness` 镜像已不再解析，而它们记录的提交在任何可达仓库中都不存在：`shigma/cosmokit` 与 `shigma/schemastery` 存在，却不含这些 SHA。没有人注意到，因为没有任何东西尝试过。
...
工具的第一次运行就找出了这两行不可达的记录，而它们是按内容修复的，而不是靠猜测某个发布标签。施加已记录的 `.ts` 说明符与 JSDoc 修改后，`vendor/cosmokit/src` 与 `shigma/cosmokit@02e691c5aa7f` 逐字节一致；`vendor/schemastery/src/index.ts` 与 `shigma/schemastery@cf0b7e5481d0`（`packages/core`）也一致，只差修改 10 所拥有的两行——`type Dict` 导入与 ESM 默认导出。两行现在都指向一个可解析的仓库，以及一个确实包含内置副本所携带源码的提交，而 `vendor/README.md` 记下了这套重新推导的方法，以备下一次上游消失。
...
- **把 `cosmokit`/`schemastery` 行改指与 Version 列匹配的 `shigma/*` 发布标签** —— 否决：标签只是对镜像所持快照的猜测。把内置源码与候选提交逐一比对只需一次 diff，就能得到该行可以声称的提交，这也正是最终落地的做法。继续让这两行指向已消失的镜像同样被否决，理由与本工具存在的理由相同：无人核对的不可验证 provenance，正是当初漂移的起点。

## Consequences

- `pnpm run vendor:status` 是同步流程要求执行的第一件事，因此一次同步从上游提交清单开始，而不是从某人手工拼出的差异开始。
- 现在九行全部可解析且报告为最新，因此命令以零码退出，`--check` 也就值得排入计划任务。某一行若不再可解析，会成为比较失败，其失败 `--check` 的方式与落后相同。
