...
`events.ts` 与 `fiber.ts` 的重新施加走的是一次真正的三方合并——以 manifest SHA 处的上游版本为 base、内置文件为 ours、上游 head 为 theirs——而不是并排阅读两份 diff。本地修改的体量已经大到：一个落在未被触碰区域的上游 hunk 很容易被肉眼漏掉，而合并会精确报出上游改动与本地改动重叠的每一处。两个文件各产生一处冲突，且都落在某项本地修改所拥有的区域内。

上游所废弃、而 harness 仍然需要的那一项行为在本地继续受支持：`events.dispatch()` 返回已解析的监听器集合，harness 有十处调用点自行驱动监听器，使单个抛出或拒绝无法饿死其余监听器。上游的 `emit()` 仍在一个不做隔离的循环里调用监听器，因此并不存在可迁移的目标；本地修改 19 去掉该标记，并记录下上游的意图。

本次同步的终点是：`cordis/src` 完整对应 `8cc9e33fab69`、manifest 的该列被推进、in-flight 一节被删除。`vendor/README.md` 中的流程现已记录三方合并、分批落地的规则，以及「每个搬运过来的修复都必须由一个缺了它就会失败的测试钉住」这一要求。

manifest 的 Version 列与内置各 `package.json` 的版本在本次同步之前就已经分叉：内置包作为一个发布族发布，因此它们的版本随 harness 前进，而该列记录的是上游快照。此前 manifest 声称两者都与上游一致，现在它写明了哪个是哪个。
...
- 被包装 fiber 的修复所关闭的是一条静默丢配置的路径，而不只是整洁性问题：在此之前，若某个 fiber 所注入的服务正好在同一时刻重载，对它调用 `update()` 会施加**上一份**配置并报告成功。回归测试正是钉住这一对更新。
- `events.dispatch()` 在本地保持未废弃状态，因此那十处调用点无需任何抑制注释即可继续编译，迁移仍然是一个决策而非 lint 的最后期限。代价是上游的信号只存在于 JSDoc 与修改日志中。
- `symbols.caller` 取代了面向身份敏感服务的「保留 shadow」机制。`ctx.logger` 是当前 harness 中唯一这样的服务，它现在显式读取自己的调用者，而不再从 tracker 必须保留的一个 shadow 中推断。想要获取调用者身份的服务不再需要 `noShadow`。
