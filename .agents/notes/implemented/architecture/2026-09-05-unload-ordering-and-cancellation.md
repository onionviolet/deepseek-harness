...
Every harness site that announced disposal from a disposer moves onto the signal and keeps its teardown in the effect. An audit of the provider services — the only fibers whose disposers now run late — found seven such sites and migrated six: the agent loop's `FactoryOwnership` (stop accepting, abort the teardown signal, wake `waitWhileActive`) and its two owner-unload aborts, `dsh-e2b` (`disposed`), `dsh-subprocess-e2b` (`disposing` plus aborting terminal setups in flight), `dsh-plan-mode` (a review outliving its fiber), `dsh-session-title` (abandon in-flight titles, then drain), and the client `conversation` service (stop handing out image URLs). The seventh, the client runner's timer wrapper, stays as it is: its flag is local to one scheduled callback whose own disposer clears the timer, and nothing observes it across an await. `docs/defensive-patterns.md` states the rule for the next service.

## Consequences

- A consumer's cleanup can use a service it injected: the provider's resources outlive it. A provider that abandons in-flight work at unload still does so immediately — `packages/session/session-title/tests/service-contracts.spec.ts` pins that behind a consumer that is still draining, and fails without the migration. `packages/boot/app-boot/tests/cordis-fiber.spec.ts` pins the ordering, and pins that the signal fires before the first disposer; both fail against the pre-redesign source.
