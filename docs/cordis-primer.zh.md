...
`ctx.waterfall` 是环绕中间件。监听器接收 `(...args, next)`。调用 `next()` 会执行下游监听器；下游返回值通过 `next()` 返回当前包装层，可由该层包装后继续向外返回。不调用 `next()` 直接返回则短路。每个 `next()` 只执行一次后续链路；再次调用它或调用外层监听器的 `next()` 都会抛出异常。
