...
// Fixture package names, deliberately not the vendored ones: the parser reads
// the table's shape, and a real upstream name in a quoted token here would be
// rewritten by `rescope-vendor`.
const manifest = [
  '| Directory | npm name | Upstream name | Version | Upstream repo | Commit |',
  '|---|---|---|---|---|---|',
  '| `widget/` | `@deepseek-ai/widget` | `widget` | 1.8.1 | https://github.com/owner/widget | `16f6fc058ade66e8ac5da0033d35a8d0f279f544` |',
  '| `gadget/` | `@deepseek-ai/gadget` | `gadget` | 4.0.0-rc.9 | https://github.com/owner/gadget (`packages/core`) | `303cfd21e41aa4168d83419efe4196c414cce3d2` |',
].join('\n')

const gadget: VendorRow = {
  directory: 'gadget',
  repository: 'owner/gadget',
...
describe('vendor manifest parsing', () => {
  ...
  it('rejects a row whose repository cell names no GitHub repository', () => {
    const broken = manifest.replace('https://github.com/owner/widget', 'an internal mirror')
  ...
  it('parses the manifest this repository ships', () => {
    ...
    expect(rows.some(row => row.repository === 'cordiverse/cordis' && row.packagePath === 'packages/core')).toBe(true)
    expect(rows.every(row => /^[\w.-]+\/[\w.-]+$/.test(row.repository))).toBe(true)
    expect(rows.every(row => /^[0-9a-f]{40}$/.test(row.commit))).toBe(true)
  })
})

describe('upstream source prefixes', () => {
  it('scopes a package inside a monorepo to its own source directory', () => {
    expect(sourcePrefix(gadget)).toBe('packages/core/src/')
  })

  it('scopes a repository-root package to its source directory', () => {
    expect(sourcePrefix({ ...gadget, packagePath: '' })).toBe('src/')
  })
})
...
describe('drift reporting', () => {
  ...
  it('keeps only the commits that reach the vendored source', () => {
    expect(commitsTouchingSource(gadget, compared)).toEqual([
  ...
  it('reports a behind row with its commits, and an up-to-date row without', async () => {
    const statuses = await collectStatus([gadget, { ...gadget, directory: 'sprocket', packagePath: 'packages/sprocket' }],
      () => Promise.resolve(compared))
    expect(formatReport(statuses)).toEqual([
      'gadget: 1 upstream commit(s) touch packages/core/src/ since 303cfd2',
      '  1c1a10e fix(core): dispatch symbol events',
      'sprocket: up to date with owner/gadget at 303cfd2',
    ])
  })

  it('keeps a failed comparison to its own row', async () => {
    const statuses = await collectStatus([gadget], () => Promise.reject(new Error('GET compare returned 404')))
    expect(statuses[0]?.error).toBe('GET compare returned 404')
    expect(formatReport(statuses)).toEqual(['gadget: comparison failed — GET compare returned 404'])
  })
})
